...
<head>
    ...
    <title>EENG | lecturer login</title>
</head>
<body>
    <div class="flex items-center justify-center w-full h-full bg-green-200">
        <form action = "/lecturer/login" method = "POST" class="flex flex-col justify-around bg-white shadow-md rounded w-1/4 h-1/2 px-8 pt-10 pb-8 mb-4">
            @csrf
            ...
            <input value = "{{old('id')}}" name = "id" class="appearance-none bg-gray-100 rounded w-full h-12 py-2 px-3 text-gray-700 leading-tight focus:outline-green-300" id="regNo" type="text" placeholder="registration number" autocomplete="off" autofocus>
            @if($errors)
                @error('regno')
          ...
          <div class="mb-6">
            ...
            <input value = "{{old('password')}}" name = "password" class="appearance-none bg-gray-100 rounded w-full h-12 py-2 px-3 text-gray-700 leading-tight focus:outline-green-300 transition-all" id="password" type="password" placeholder="password">
            @if($errors)
                @error('loginpassword')
                <p class = "text-red-400">
                    {{$message}}
